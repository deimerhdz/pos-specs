...
- **Logging operativo actual (request_id → Sentry)**: hoy no está aplicado al módulo de órdenes (solo a un módulo administrativo distinto). Este spec no construye esa capa operativa — solo agrega la capa de auditoría descrita aquí, que es independiente. **[Revertido en una extensión posterior de este mismo spec — ver Clarifications, segunda ronda, y FR-015 en adelante: el patrón `request_id` se extiende a todo el backend salvo super-admin.]**
...
- **Segunda extensión post-implementación (logging operativo)**: con el feature ya en producción, el usuario pidió además un log operativo general con el patrón `request_id` (hoy solo en `/api/v1/super-admin`) para depurar incidentes en producción en todo el backend — no solo en órdenes. Esta capacidad es de naturaleza distinta a la auditoría de negocio de FR-001–FR-014 (trazabilidad técnica de cualquier acción mutativa, no eventos de dominio curados), pero por decisión explícita del usuario se incorpora **a este mismo spec** en vez de crear uno nuevo. Ver FR-015 en adelante.
...
### Session 2026-09-03 (segunda ronda — extensión de logging operativo)

- Q: El feature ya está implementado y en producción. Lo pedido ahora (logging operativo con `request_id` de todas las acciones en el menú QR y la Terminal de Mesas) es una capacidad más amplia y de otra naturaleza que la auditoría de negocio ya construida. ¿Se trata como spec nuevo o como extensión de este spec? → A: Extensión de este mismo spec (074), no un spec nuevo.
- Q: "Todas las acciones que hace el usuario" — ¿incluye peticiones de solo lectura (ver menú, listar mesas, polling de tiempo real), o solo las que cambian algo? → A: Solo acciones mutativas (`POST`/`PUT`/`PATCH`/`DELETE`); las de solo lectura (`GET`/`HEAD`/`OPTIONS`) quedan fuera, incluido el polling de tiempo real.
- Q: ¿Qué routers del backend cuentan como "el menú" y "la Terminal de Mesas" para este logging? → A: Todo el backend de negocio salvo `/api/v1/super-admin` (que ya tiene este patrón) — no se acota a una lista curada de routers (cart/menu/orders/cash/table-sessions), sino a "cualquier ruta mutativa que no sea super-admin".
- Q: Con ese alcance tan amplio (incluye `auth`, cambio/recuperación de contraseña, `users`), ¿el log debe incluir el cuerpo completo de la petición, o solo metadatos estructurados? → A: Solo metadatos estructurados (método, ruta, status, duración, actor, tenant, `request_id`) — nunca el cuerpo de la petición ni de la respuesta, sin excepción y sin necesidad de mantener una lista de campos sensibles a excluir.
- Q: ¿El nombre/etiqueta de cada entrada de este log se cura a mano por endpoint (como los 8 eventos de orden), o se deriva automáticamente del método + la ruta? → A: Se deriva automáticamente del método y la ruta registrada — sin curación manual por endpoint, para que cubra cualquier ruta nueva sin tener que tocar este feature.
- Q: Las 8 rutas que ya disparan un evento de auditoría de orden también son mutativas — ¿el log operativo genérico también las registra (dos entradas por esa petición), o las omite porque spec 074 ya las cubre? → A: Las registra también, sin excepción ni lista de omisión — los 8 eventos de auditoría de orden (FR-001–FR-014) se **mantienen exactamente como están**, como una capa de negocio adicional sobre el log técnico genérico, no como un reemplazo. Se descartó explícitamente la alternativa de eliminarlos y dejar que el log genérico los reemplazara, por la pérdida de capacidad ya construida y probada (tipado de actor, hash de datos sensibles, detalle de negocio) que eso implicaba.
...
### User Story 4 - Depurar un incidente en producción con trazabilidad técnica de cualquier acción (Priority: P2)

Un desarrollador o administrador necesita, ante un reporte de "algo falló" en el menú QR o en la Terminal de Mesas (sin que necesariamente sea una de las 8 transiciones de orden ya auditadas — puede ser un login, una actualización de categoría, una promoción, un método de pago, etc.), poder ubicar en Sentry qué petición mutativa ocurrió, con qué resultado (status) y en qué momento, correlacionando por `request_id` — igual que ya puede hacerlo hoy para el módulo de super-admin, pero para el resto del backend.

**Why this priority**: Es una mejora de observabilidad operativa general — valiosa para depurar producción, pero no resuelve una disputa de dinero como las User Stories 1-3 (que siguen siendo la razón de ser original del spec); por eso es P2, no P1.

**Independent Test**: Se puede probar ejecutando una acción mutativa cualquiera fuera de las 8 ya auditadas (p. ej. crear una categoría, actualizar un método de pago) y verificando que aparece en Sentry una entrada con método, ruta, status, duración, actor, tenant y `request_id` — sin el cuerpo de la petición — y que una petición de solo lectura (p. ej. listar categorías) no genera ninguna entrada.

**Acceptance Scenarios**:

1. **Given** un cajero actualiza un método de pago desde la Terminal, **When** la petición se completa, **Then** aparece en Sentry una entrada con método `PUT`/`PATCH`, la ruta, el status de la respuesta, la duración, el actor (cajero) y el `request_id` de esa petición — sin el cuerpo enviado.
2. **Given** un comensal consulta el menú (`GET`), **When** la petición se completa, **Then** no aparece ninguna entrada de este log para esa petición.
3. **Given** una orden se crea, se confirma y se paga (las 8 transiciones ya auditadas), **When** se revisan los logs de esas peticiones en Sentry, **Then** aparecen **ambas** capas para cada una: la entrada técnica genérica (método/ruta/status/duración/`request_id`) y el evento de auditoría de orden específico (`order.created`, `order.confirmed`, etc., con su actor tipado y sus datos hasheados) — ninguna reemplaza a la otra.
...
- ¿Qué pasa con una petición mutativa a `/api/v1/super-admin`? No la cubre este spec — ese módulo ya tiene su propio patrón de `request_id`/reporte de errores desde antes (spec 068); este spec no lo duplica ni lo reemplaza.
- ¿Qué pasa con una petición mutativa que falla (4xx/5xx)? Se registra igual que una exitosa — método, ruta, status (el de error), duración, actor, tenant y `request_id` — precisamente porque los fallos son el caso de uso principal para depurar producción (FR-015).
- ¿Qué pasa si una petición mutativa no tiene un actor autenticado resoluble (p. ej. falla la propia autenticación)? El log igual se registra, con el actor que sí se pueda determinar en ese punto (o sin él, si la petición nunca llegó a resolver ninguno) — nunca se bloquea el registro del log por no tener un actor completo, a diferencia de los eventos de auditoría de orden (FR-003), que si exigen un actor exhaustivo.
...
**Los siguientes requisitos (FR-015 en adelante) corresponden a la extensión de logging operativo (Clarifications, segunda ronda; User Story 4) — trazabilidad técnica general, distinta de la auditoría de negocio de FR-001 a FR-014, que se mantiene sin cambios.**

- **FR-015**: El sistema DEBE registrar, hacia la misma herramienta externa de monitoreo, una entrada de log por cada petición HTTP mutativa (`POST`/`PUT`/`PATCH`/`DELETE`) recibida en cualquier ruta de negocio **excepto** `/api/v1/super-admin` (que ya tiene un patrón equivalente desde antes de este spec) — sin importar si esa petición terminó en éxito o en error.
- **FR-016**: Las peticiones de solo lectura (`GET`/`HEAD`/`OPTIONS`) NO DEBEN generar una entrada de este log, incluyendo los endpoints de sondeo/tiempo real.
- **FR-017**: Cada entrada de este log DEBE incluir, como mínimo, atributos estructurados equivalentes a: método HTTP, ruta, código de estado de la respuesta, duración de la petición, un identificador del actor que la ejecutó (o su ausencia, si no se pudo resolver ninguno), el tenant/negocio activo, y un identificador de solicitud (`request_id`) único por petición.
- **FR-018**: Esta entrada de log NO DEBE incluir el cuerpo (payload) de la petición ni de la respuesta, bajo ninguna circunstancia — ni siquiera parcialmente ni con una lista de campos excluidos por endpoint. Solo viajan los metadatos estructurados de FR-017.
- **FR-019**: La etiqueta/nombre de cada entrada de este log DEBE derivarse automáticamente del método HTTP y la ruta de la petición, sin requerir una definición manual por endpoint — a diferencia del catálogo curado de 8 tipos de evento de FR-001.
- **FR-020**: Este log operativo general DEBE convivir con los eventos de auditoría de orden (FR-001–FR-014) sin reemplazarlos: una petición que ya dispara uno de esos 8 eventos DEBE generar también su entrada de log operativo genérico correspondiente — ambas capas coexisten para la misma petición, cada una con su propósito (trazabilidad técnica vs. auditoría de negocio).
- **FR-021**: Cada uno de los 8 eventos de auditoría de orden (FR-001–FR-014) DEBE incluir, además de lo ya exigido (order_id, tenant, actor, etc.), el `request_id` de la petición HTTP que lo originó, cuando esa petición pase por una ruta cubierta por FR-015 — permitiendo correlacionar el evento de negocio con su entrada de log operativo genérico. Cuando el evento se origina fuera de una petición HTTP directa (p. ej. la confirmación automática de una orden disparada dentro de otra petición), el `request_id` es el de esa petición que lo originó, no uno nuevo.
...
- **Entrada de log operativo** (FR-015 en adelante): entrada de log técnica, sin curar, enviada a la misma herramienta externa de monitoreo, que representa una petición HTTP mutativa cualquiera (no solo las de orden). Incluye método, ruta, status, duración, actor (si se pudo resolver), tenant, y `request_id`. Nunca incluye el cuerpo de la petición ni de la respuesta. Es una entidad distinta del "Evento de auditoría de orden": más genérica, sin tipado de negocio, y cubre todo el backend salvo super-admin — no solo las 8 transiciones de una orden.
...
- **SC-006**: El 100% de las peticiones mutativas (`POST`/`PUT`/`PATCH`/`DELETE`) a rutas fuera de `/api/v1/super-admin`, ejecutadas durante pruebas de aceptación, generan una entrada de log operativo con método/ruta/status/duración/`request_id`; el 0% de las peticiones de solo lectura genera una.
- **SC-007**: El 0% de las entradas de log operativo generadas durante pruebas de aceptación contienen el cuerpo de la petición o de la respuesta.
- **SC-008**: Para cualquiera de las 8 transiciones de orden ya auditadas, dado el `request_id` de la petición que la originó, el staff puede ubicar en la herramienta externa de monitoreo tanto la entrada de log operativo genérica como el evento de auditoría de orden específico de esa misma petición.
...
- Decisión de alcance (ver Clarifications, segunda ronda): el logging operativo general (FR-015 en adelante) es una extensión deliberada de este mismo spec, aunque sea de naturaleza distinta a la auditoría de negocio original (FR-001–FR-014) — el usuario decidió explícitamente no dividirlo en un spec separado. Los 8 eventos de auditoría de orden ya implementados NO se modifican, no se eliminan y no se reemplazan por este log genérico; ambas capas conviven.
- El logging operativo general se limita a peticiones mutativas (`POST`/`PUT`/`PATCH`/`DELETE`); las de solo lectura, incluido el sondeo de tiempo real, quedan fuera de alcance — se asume que el volumen y el valor de depuración de las lecturas no justifica su costo en Sentry Logs.
- El contenido de cada entrada de log operativo se limita a metadatos estructurados (método, ruta, status, duración, actor, tenant, `request_id`); nunca el cuerpo de la petición/respuesta — se asume que esto es suficiente para depurar la mayoría de incidentes de producción (¿qué pasó, con qué resultado, cuándo, quién?), aceptando que no reemplaza una inspección más profunda cuando se necesite ver el contenido exacto enviado.
- La etiqueta de cada entrada se deriva automáticamente del método y la ruta (no hay un catálogo curado de nombres de evento por endpoint, a diferencia de los 8 eventos de orden) — se asume que esto es suficiente para el propósito de depuración técnica de esta capacidad, y que curar un nombre de negocio por cada una de las decenas de rutas mutativas del backend no se justifica frente al valor que aporta.
- La ventana de retención de este log operativo general es la misma que la del resto de este spec (la del plan contratado de la herramienta externa de monitoreo, 7 a 30 días, FR-013) — no se introduce una retención distinta para esta capa.
