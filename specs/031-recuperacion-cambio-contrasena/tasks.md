...
- [X] T001 [P] Agregar `PASSWORD_RESET_MAX_REQUESTS` (default `3`), `PASSWORD_RESET_WINDOW_SECONDS`
...
- [X] T002 [P] Crear `../pos-backend/app/characterization_tests/auth_fixtures.py`: extiende el
...
- [X] T003 Agregar el modelo `PasswordResetToken` nuevo en `../pos-backend/app/core/models.py`
...
- [X] T004 Agregar la columna `tokens_valid_after: DateTime | None` (nulable, sin default) al
  modelo `User` en `../pos-backend/app/core/models.py` — ver
  [data-model.md](./data-model.md#user-sharedusers--modificada).
- [X] T005 Crear la migración Alembic nueva en
...
- [X] T006 [P] Agregar `enforce_sliding_window(key, limit, window_seconds)` nueva en
...
- [X] T007 [P] Agregar `password_changed_email_body(when, email)` nueva en
...
- [X] T008 [P] Agregar el chequeo `token_data["iat"] < user.tokens_valid_after` (→ `401` con detalle
...
- [X] T009 [P] Agregar el mismo chequeo en el handler de `GET /auth/refresh-token`,
...
- [X] T010 Crear `../pos-backend/app/characterization_tests/test_auth_session_revocation.py` con
...
- [X] T011 [P] [US1] Agregar `ForgotPasswordRequest` (`email: EmailStr`) y `ResetPasswordRequest`
...
- [X] T012 [P] [US1] Agregar `password_reset_email_body(reset_url, expiry_minutes)` nueva en
...
- [X] T013 [US1] Implementar `POST /auth/forgot-password` en
...
- [X] T014 [US1] Implementar `GET /auth/reset-password/validate` en
...
- [X] T015 [US1] Implementar `POST /auth/reset-password` en
...
- [X] T016 [P] [US1] Crear `../pos-backend/app/characterization_tests/test_auth_forgot_password.py`
...
- [X] T017 [P] [US1] Crear `../pos-backend/app/characterization_tests/test_auth_reset_password.py`
...
- [X] T018 [P] [US1] Agregar `ForgotPasswordRequest`, `ResetPasswordRequest`,
  `ValidateResetTokenResponse` en `../pos-heladeria/src/app/core/auth/auth.models.ts`.
- [X] T019 [US1] Agregar `forgotPassword()`, `validateResetToken()`, `resetPassword()` en
  `../pos-heladeria/src/app/core/auth/auth-api.service.ts`. Depende de T018.
- [X] T020 [US1] Agregar `forgotPassword()`/`resetPassword()` en
  `../pos-heladeria/src/app/core/services/auth.service.ts`. Depende de T019.
- [X] T021 [P] [US1] Crear `../pos-heladeria/src/app/modules/auth/pages/forgot-password.component.ts`
  (campo "Correo electrónico" + botón "Enviar enlace", mensaje genérico de FR-003, funcional en
  móvil FR-024). Depende de T020.
- [X] T022 [P] [US1] Crear `../pos-heladeria/src/app/modules/auth/pages/reset-password.component.ts`
  ...
  Depende de T020.
- [X] T023 [P] [US1] Agregar el enlace "Restablecer contraseña" bajo el campo de contraseña en
  `../pos-heladeria/src/app/modules/auth/pages/login.component.ts`, visible sin scroll a 375px de
  ancho (FR-001).
- [X] T024 [US1] Agregar las rutas `forgot-password` (guard `redirectIfAuthGuard`, igual que
...
- [X] T025 [US2] Cambiar `ChangePasswordRequest.new_password` de
...
- [X] T026 [US2] Modificar `POST /auth/change-password` en
  ...
  T007, T025.
- [X] T027 [P] [US2] Crear `../pos-backend/app/characterization_tests/test_auth_change_password.py`
...
- [X] T028 [US2] Completar `../pos-backend/app/characterization_tests/test_auth_session_revocation.py`
...
- [X] T029 [P] [US2] Cambiar `Validators.minLength(8)`/`maxLength(12)` (antes 6/128) en
...
- [X] T031 [P] [US2] Agregar la opción "Cambiar contraseña" en el dropdown de usuario de
  `../pos-heladeria/src/app/modules/dashboard/layout/header.component.ts`, junto a "Cerrar
  sesión".
- [X] T032 [US2] Agregar la ruta `dashboard/mi-cuenta` (sin `roleGuard`) en
...
- [X] T035 [P] Extender `../pos-heladeria/src/app/core/auth/auth-api.service.spec.ts` con specs
  para `forgotPassword()`, `validateResetToken()`, `resetPassword()`.
- [X] T036 [P] Extender `../pos-heladeria/src/app/core/services/auth.service.spec.ts` con specs
  para `forgotPassword()`/`resetPassword()`.
- [X] T037 Ejecutar la suite completa de backend
...
- [X] T038 Validación manual de frontend según quickstart.md §"Frontend — validación manual":
